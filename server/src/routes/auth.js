...
const getSupabaseConfigError = () => {
  ...
  const placeholders =
    /your-project|your_.*key|your_service|placeholder/i.test(url) ||
    /your-project|your_.*key|your_service|placeholder/i.test(serviceKey);

  if (missing || placeholders) {
    return 'Supabase is not configured. Update SUPABASE_URL and SUPABASE_SERVICE_KEY in server/.env.';
  }
